...
const { classifyFiling, selectLatestArticles, articlesFilename, buildZip, crc32 } = require('./articles');
...
test('buildZip produces a structurally valid store-only archive', () => {
  const a = Buffer.from('%PDF-1.4 hello');
  const b = Buffer.from('%PDF-1.4 world!!');
  const zip = buildZip([{ name: 'a.pdf', data: a }, { name: 'b.pdf', data: b }]);

  assert.equal(zip.readUInt32LE(0), 0x04034b50);                 // first local header
  assert.equal(zip.readUInt32LE(zip.length - 22), 0x06054b50);   // end of central directory
  assert.equal(zip.readUInt16LE(zip.length - 22 + 10), 2);       // total entries
  ...
  assert.equal(cdOffset, 30 + 5 + a.length + 30 + 5 + b.length); // both local entries precede it
  assert.equal(zip.readUInt32LE(14), crc32(a));                  // crc stored in first local header
  assert.equal(zip.subarray(30, 35).toString(), 'a.pdf');
});
